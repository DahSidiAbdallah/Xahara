      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-sm' : 'bg-white'}`}>
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center">
            <button
              className={`flex items-center text-2xl font-bold text-primary focus:outline-none ${i18n.language === 'ar' ? 'flex-row-reverse ml-12' : 'mr-12'}`}
              onClick={() => {
            ...
            >
              {i18n.language === 'ar' ? (
                <>
                  <span style={{letterSpacing: '0.05em'}}>AHARA</span>
                  <img src={xLogo} alt="XAHARA Logo" className="h-8 w-8 object-contain ml-2" />
                </>
              ) : (
                <>
                  <img src={xLogo} alt="XAHARA Logo" className="h-8 w-8 object-contain mr-2" />
                  <span style={{letterSpacing: '0.05em'}}>AHARA</span>
                </>
              )}
            </button>
